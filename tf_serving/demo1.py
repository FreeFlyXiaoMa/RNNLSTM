...
def wide_model(input_data):
    ...
    with tf.name_scope("wide"):
        #修正的方式初始化权重，输出层节点只有一个
        weights=tf.Variable(tf.truncated_normal([input_len,1],
                                                stddev=1.0/math.sqrt(input_len)),name='weights')
...
def deep_model(input_data,hidden1_units,hidden2_units,hidden3_units):
    ...
    """
    #得到每个样本的维度
    input_len=int(input_data.shape[1])  #shape==1表示列数
    ...
    with tf.name_scope('hidden2'):
        ...
        weights=tf.Variable(tf.truncated_normal([hidden1_units,hidden2_units],
                                               stddev=1.0/math.sqrt(float(input_len))),name='weights2')
    ...
    with tf.name_scope('hidden3'):
        ...
        weights=tf.Variable(tf.truncated_normal([hidden2_units,hidden3_units],
                                                stddev=1.0/math.sqrt(float(input_len))),name='weights3')
        biases=tf.Variable(tf.zeros[hidden3_units],name='biases3')
        hidden3=tf.nn.relu(tf.matmul(hidden2,weights))+biases

    with tf.name_scope('output'):
        #修正的方式初始化权重
        weights=tf.Variable(tf.truncated_normal([hidden3_units,1],stddev=1.0/math.sqrt(float(input_len))),
                            name='weights4'
                            )
...
def build_and_saved_wdl():
    ...
    x_deep_data=x_deep_data.reshape(shape=[-1,10])

    x_wide_data=np.random.rand(10000)
    x_wide_data=x_wide_data.reshape(shape=[-1,10])

    x_deep=tf.placeholder(tf.float32,shape=[None,10])   #占位符
    x_wide=tf.placeholder(tf.float32,shape=[None,10])

    y=tf.placeholder(tf.float32,shape=[None,1])

    y_data=np.array([random.randint(0,1) for i in range(1000)])
    y_data=y_data.reshape(shape=[-1,1])

    #没有设置验证集
    train_step,loss,prediction=build_wdl(x_deep,x_wide,y)
    sess=tf.Session()
    sess.run(tf.global_variables_initializer())
    sess.run(train_step,feed_dict={x_deep:x_deep_data,
                                   x_wide:x_wide_data,
                                   y:y_data})
    #将训练好的模型保存在当前文件夹下
    builder=tf.saved_model.builder.SavedModelBuilder(join('./tf_serving',MODEL_VERSION))
    inputs={
    ...
    def test_servable_api():
        ...
        """
        #随机产生10条数据
        x_deep_data=np.random.rand(100).reshape(shape=[-1,10])
        x_wide_data=np.random.rand(100).reshape(shape=[-1,10])
